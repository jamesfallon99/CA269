...
class Customer {
    ...
    public String htmlStatement()
    {
        double totalAmount =0;
        int frequent=0;
        String results="<h1>Statement for " + _name + "</h1>\n<ol>\n";
        for (Rental rental:_rentals){
           
            results+="  <li>"+rental.getMovie().getTitle()+"  "+String.valueOf(getTotal(rental))+"</li>\n";
           
        }
        results += "</ol>\n<p>Amount owed is " +String.valueOf(getTotalCharge()) +"</p>\n";
        results += "<p>You earned " + String.valueOf(getTotalFrequentRenterPoints())+" frequent renter points."+"</p>\n";
        return results;
    }
